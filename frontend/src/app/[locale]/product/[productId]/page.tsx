...
const ProductPage = () => {
  ...
  return (
    <div className="small-responsive-container flex flex-1 flex-col items-center gap-8 py-4 md:flex-row md:items-start md:justify-between">
      <ImageGallery />
      <div className="flex flex-col gap-2 lg:col-span-2">
        ...
        <div className="flex gap-4">
          <button
            ...
            className="rounded bg-orange-500 px-6 py-2 text-white transition-colors hover:bg-orange-600 disabled:bg-orange-200"
            disabled={quantity < 1}
          >
            Add to Cart
          </button>
        ...
        <div>
          ...
          {cart?.cartItems.find((item: any) => item.product === productId)
            ?.qty > 0 && (
            <span>{`(Added to cart: ${cart?.cartItems.find((item: any) => item.product === productId)?.qty})`}</span>
          )}
        </div>
