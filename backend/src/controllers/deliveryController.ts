...
export class DeliveryController {
    ...
    static async createDelivery(req: Request, res: Response): Promise<void> {
        const { order, trackingNumber, shippingMethod, estimatedDeliveryDate } = req.body;
        try {
    ...
    static async getDeliveryById(req: Request, res: Response): Promise<void> {
        const { deliveryId } = req.params;
        try {
            const delivery = await DeliveryModel.findById(deliveryId).populate('order shippingMethod');
            if (!delivery) {
                res.status(404).json({ message: 'Delivery not found' });
                return;
            }
